...
async fn handle_chat_completion(
    ...
    let goal = payload.messages.last()
    ...
    let task_id = scheduler.run_goal(goal).await.unwrap_or_else(|_| Uuid::nil());
    
    // Grab the final response from memory logs (mocked for simplicity here, normally we'd return the final assistant message)
    let _logs = state.memory.get_task_logs(&task_id.to_string()).unwrap_or_default();
